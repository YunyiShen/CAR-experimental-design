...
for(i_rep in 1:n_reps){
  ...
  posterior_para_Omega_wrc <- getMAP_Omega_Wishart(Y_init, X_init, B0 %*% Sigma, phi, 2*lambda, Lambda1)
  posterior_para_Omega_wrc_noexp <- getMAP_Omega_Wishart(Y_no_exp, 0*X_init, B0 %*% Sigma, phi, 2*lambda, Lambda1)
  posterior_para_Omega_wru <- getMAP_Omega_Wishart(Y_init, X_init, B0 %*% Sigma, phi, 2*lambda, Lambda2)
  posterior_para_Omega_wru_noexp <- getMAP_Omega_Wishart(Y_no_exp, 0*X_init, B0 %*% Sigma, phi, 2*lambda, Lambda2)
  
  
  posterior_para_Omega_mrc <- getMAP_Omega_MGIG(Y_init, X_init, B0, phi, psi,lambda, Lambda1)
  posterior_para_Omega_mrc_noexp <- getMAP_Omega_MGIG(Y_no_exp, 0*X_init, B0, phi, psi,lambda, Lambda1)
  posterior_para_Omega_mru <- getMAP_Omega_MGIG(Y_init, X_init, B0, phi, psi,lambda, Lambda2)
  posterior_para_Omega_mru_noexp <- getMAP_Omega_MGIG(Y_no_exp, 0*X_init, B0, phi, psi,lambda, Lambda2)
  

  
  
  #posterior_para_Omega_nex <- getMAP_Omega_Wishart(Y_no_exp, 0*X_init, B0 %*% Sigma, phi, lambda, Lambda2)
  ...
  MGIG_random_cert[i_rep,1] <- stein_loss(
                                            posterior_para_Omega_mrc, Omega
                                            ) /
                              stein_loss(
                                            posterior_para_Omega_mrc_noexp,Omega
                                            )
  MGIG_random_uncert[i_rep,1] <- stein_loss(
                                            posterior_para_Omega_mru, Omega
                                            ) /
                                stein_loss(
                                            posterior_para_Omega_mru_noexp, Omega
                                            )
  
  Wishart_random_cert[i_rep,1]  <- stein_loss(
                                                posterior_para_Omega_wrc,Omega
                                                ) / 
                                  stein_loss(
                                                posterior_para_Omega_wrc_noexp,Omega
                                                )
  Wishart_random_uncert[i_rep,1] <- stein_loss(
                                                posterior_para_Omega_wru,Omega
                                                ) / 
                                    stein_loss(
                                                posterior_para_Omega_wru_noexp,Omega
                                                )
  ...
  for(i_step in 1:n_steps + 1){
    ...
    # Result for MGIG
    posterior_para_Omega_wrc <- getMAP_Omega_Wishart(Y_r, X_r, B0 %*% Sigma, phi, 2*lambda, Lambda1)
    posterior_para_Omega_wrc_noexp <- getMAP_Omega_Wishart(Y_no_exp, 0*X_r, B0 %*% Sigma, phi, 2*lambda, Lambda1)
    posterior_para_Omega_wru <- getMAP_Omega_Wishart(Y_r, X_r, B0 %*% Sigma, phi, 2*lambda, Lambda2)
    posterior_para_Omega_wru_noexp <- getMAP_Omega_Wishart(Y_no_exp, 0*X_r, B0 %*% Sigma, phi, 2*lambda, Lambda2)
    
    # result for Wishart
    posterior_para_Omega_mrc <- getMAP_Omega_MGIG(Y_r, X_r, B0, phi, psi,lambda, Lambda1)
    posterior_para_Omega_mrc_noexp <- getMAP_Omega_MGIG(Y_no_exp, 0*X_r, B0, phi, psi,lambda, Lambda1)
    posterior_para_Omega_mru <- getMAP_Omega_MGIG(Y_r, X_r, B0, phi, psi,lambda, Lambda2)
    posterior_para_Omega_mru_noexp <- getMAP_Omega_MGIG(Y_no_exp, 0*X_r, B0, phi, psi,lambda, Lambda2)
    
    
    # save
    MGIG_random_cert[i_rep,i_step] <- stein_loss(
                                            posterior_para_Omega_mrc,Omega
                                            ) /
                                stein_loss(
                                            posterior_para_Omega_mrc_noexp,Omega
    ...
    MGIG_random_uncert[i_rep,i_step] <- stein_loss(
                                            posterior_para_Omega_mru,Omega
                                            ) /
                                stein_loss(
                                            posterior_para_Omega_mru_noexp,Omega
    ...
    Wishart_random_cert[i_rep,i_step] <- stein_loss(
                                                posterior_para_Omega_wrc,Omega
                                                ) / 
                                    stein_loss(
                                                posterior_para_Omega_wrc_noexp,Omega
    ...
    Wishart_random_uncert[i_rep,i_step] <- stein_loss(
                                                posterior_para_Omega_wru,Omega
                                                ) / 
                                    stein_loss(
                                                posterior_para_Omega_wru_noexp,Omega
